...
const projectsMeta: Omit<Project, 'image' | 'gallery'>[] = [
  {
    ...
    detail:
      "상명대 이캠퍼스 서버 장애로 학생들이 원인도 모른 채 빈 화면만 보던 상황에서, 상명대 주요 서비스의 접속 여부와 응답 속도를 실시간으로 보여주는 모니터링 서비스를 수일 만에 만들어 배포했습니다. 에브리타임 Hot 게시물에 오르며 누적 1,088명이 사용했고, 이캠퍼스 정상화 후 비용을 아끼려 서비스를 종료했다가 2026년 재장애 때 자발적으로 리디자인해 재오픈, 총 활성 사용자 1,245명을 확보했습니다. 기술 선택에는 매번 근거를 두었습니다. 검색 노출을 위해 CSR인 CRA를 SSR이 가능한 Next.js로 전면 이관하고, 학생들이 실제로 검색하는 표현('이캠 안됨', '상명대 서버 상태' 등)을 메타 태그·구조화 데이터·본문 카피에 심는 키워드 중심 SEO를 Lighthouse 최적화·서치콘솔 등록과 함께 진행했습니다. 그 결과 구글 '상명대 서버' 검색 3위에 올랐고, 상위 1·2위가 수강신청용 서버시간 사이트(네이비즘 계열)인 점을 감안하면 서버 상태 모니터링 서비스로는 사실상 1위입니다. 백엔드는 Render 프리티어 콜드스타트를 줄이려 Express에서 Go(표준 라이브러리)로 재작성하고, ISR·SSE·서버측 캐시로 첫 렌더 지연과 방문자 수에 따른 외부 트래픽 증가를 함께 잡았습니다. 알림은 사업자 인증이 필요한 카카오 대신 Discord 웹훅으로 구현했습니다. 최근에는 확정된 장애에 대해 같은 시간대·요일·학사일정 맥락에서의 과거 장애 빈도와 복구 시간을 SQL로 집계해 프롬프트에 근거로 주입하고, 현재 오류가 일시적일지 지속적일지를 구조화 출력(JSON)으로 판정하는 AI 분석 기능을 추가했습니다. 요청당이 아니라 장애 확정 시점에 1회만 호출해 비용을 통제하고, '통계에 없는 사실은 지어내지 말라'는 제약과 방어적 파싱으로 환각을 억제했습니다. 임베딩 컬럼은 이력이 충분히 쌓이면 유사 장애 벡터 검색으로 확장하기 위해 미리 적재해 두었습니다. 이력 백필 과정에서는 복원한 데이터의 58%가 CI 아티팩트임을 확인해 걸러냈습니다. 실제 문제에서 출발해 실사용자를 모으고, 인프라·SEO·AI까지 근거를 갖고 직접 설계·운영한 프로젝트입니다.",
    stack: [
      'Next.js',
      'Go',
      'Supabase',
    ...
    detailEn:
      "When an outage of Sangmyung University's e-Campus server left students staring at blank screens with no idea what was wrong, I built and shipped — in a matter of days — a monitoring service that shows the reachability and response time of SMU's main services in real time. It hit Everytime's Hot board and was used by 1,088 people in total. After e-Campus recovered I shut the service down to save costs, then voluntarily redesigned and relaunched it during a second outage in 2026, reaching 1,245 active users. Every technical choice had a reason behind it. For search visibility I migrated the whole front-end from CRA (client-side rendering) to Next.js (server-side rendering), and ran keyword-driven SEO on top — building the meta tags, structured data, and body copy around the phrases students actually search (\"이캠 안됨\" / is e-Campus down, \"상명대 서버 상태\") — alongside Lighthouse optimization and Search Console registration. It now ranks 3rd on Google for \"상명대 서버\" (SMU server); since the two results above it are course-registration server-time sites (Naverism-style), it's effectively 1st among actual status-monitoring services. For the back-end, I rewrote Express in Go (standard library only) to cut cold starts on Render's free tier, and used ISR, SSE, and server-side caching to tackle both first-render latency and the growth in outbound traffic that scales with visitor count. For alerts, instead of KakaoTalk — which requires business verification — I used Discord webhooks. More recently I added an AI analysis feature: when an outage is confirmed, it aggregates that service's history via SQL — how often outages have hit in the same hour-of-day, weekday, and academic-term context, and how long they took to recover — feeds those numbers into the prompt as grounding, and classifies whether the current error is likely transient or sustained via structured (JSON) output. It runs once at the moment an outage is confirmed rather than per request to control cost, and curbs hallucination with a 'state nothing that is not in the stats' constraint plus defensive parsing. An embedding column is populated in advance so this can grow into similarity search over past outages once there's enough history. During the history backfill I found that 58% of the recovered data was CI artifacts and filtered it out. It's a project that started from a real problem, drew real users, and that I designed and operated end to end — infrastructure, SEO, and AI — with a rationale for each decision.",
